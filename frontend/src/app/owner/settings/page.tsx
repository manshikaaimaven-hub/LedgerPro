...
import { isLoggedIn, getOwner } from '@/lib/auth'; // adjust to your actual exported function name
...
export default function SettingsPage() {
  ...
  const exportCsv = async () => {
    ...
    const rows = [
      ['Name', 'Phone', 'Address', 'GST Number', 'Balance'],
      ...items.map(c => [c.name, c.phone, c.address || '', c.gst_number || '', String(c.balance)]),
    ];
    ...
    a.href = url;
    a.download = `ledgerpro-customers-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
      ...
      <AppShell title="Settings" subtitle="Profile & preferences">
        <div className="flex flex-col gap-4 overflow-y-auto p-4">
          ...
          <div className="overflow-hidden rounded-card border border-line bg-surface shadow-card">
            <SettingsRow icon="ti-rotate" tone="warning" title="Restore requests" sub="Review deleted transactions customers want back" onClick={() => router.push('/owner/restore-requests')}/>
            ...
            <SettingsRow icon="ti-table-export" tone="success" title="Export data" sub="Download ledger as CSV" onClick={exportCsv} />
            <SettingsRow icon="ti-users-group" tone="brand" title="Manage manufacturers" sub="Add or switch manufacturer profile" onClick={() => alert('Coming in a later phase')} />
