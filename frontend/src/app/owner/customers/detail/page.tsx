...
// import { IconArrowDownCircle, IconArrowUpCircle, IconReceipt2, IconArrowLeft } from '@tabler/icons-react';
...
//         <button onClick={() => router.push('/customers')} className="mt-3 text-sm text-brand-blue">
//           Back to customers
//         </button>
//       </div>
//     );
...
//       <button onClick={() => router.back()} className="mb-3 flex items-center gap-1.5 text-sm text-t2">
//         <IconArrowLeft size={16} /> Back to customers
//       </button>

//       {customer && (
//         <div className="mb-4 flex items-center gap-3 rounded-card border border-[var(--bd)] bg-surface-card p-4">
//           <div className="flex h-12 w-12 items-center justify-center rounded-full bg-brand-green-light text-base font-medium text-brand-green-dark">
//             {initials(customer.name)}
//           </div>
//           <div className="min-w-0 flex-1">
//             <div className="truncate text-base font-medium">{customer.name}</div>
//             <div className="text-xs text-t2">{customer.phone}</div>
//           </div>
//           <div className={`text-lg font-medium ${customer.balance >= 0 ? 'text-brand-green' : 'text-brand-red'}`}>
//             {formatINR(customer.balance)}
//           </div>
//         </div>
//       )}

//       <div className="overflow-hidden rounded-card border border-[var(--bd)] bg-surface-card">
//         {txns.length === 0 && !loading ? (
//           <div className="p-9 text-center text-t2">
//             <IconReceipt2 size={40} className="mx-auto mb-2 text-t3" />
//             <p className="text-sm">No transactions yet.</p>
//           </div>
//         ) : (
//           txns.map(t => (
//             <div key={t.id} className="flex items-center gap-3 border-b border-[var(--bd)] px-3.5 py-3.5 last:border-none">
//               <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${t.type === 'cr' ? 'bg-brand-green-light text-brand-green' : 'bg-brand-red-light text-brand-red'}`}>
//                 {t.type === 'cr' ? <IconArrowDownCircle size={20} /> : <IconArrowUpCircle size={20} />}
//               </div>
//               <div className="min-w-0 flex-1">
//                 <div className="truncate text-sm font-medium">{t.note || (t.type === 'cr' ? 'Credit note' : 'Debit note')}</div>
//                 <div className="mt-0.5 text-xs text-t2">
//                   {formatDate(t.entry_date)}{t.invoice_number ? ` · Inv #${t.invoice_number}` : ''}
//                 </div>
//               </div>
//               <div className="text-right">
//                 <div className={`text-[15px] font-medium ${t.type === 'cr' ? 'text-brand-green' : 'text-brand-red'}`}>
//                   {t.type === 'cr' ? '+' : '-'}{formatINR(t.amount)}
//                 </div>
//                 <div className="mt-0.5 text-[10px] text-t3">Bal: {formatINR(t.running_balance)}</div>
//               </div>
//             </div>
//           ))
//         )}
...
//           disabled={loading}
//           className="mt-3 w-full rounded-input border border-[var(--bd2)] py-3 text-sm font-medium disabled:opacity-60"
//         >
