...
# ───────────────────────────────────────────────────────────────
def create_customer_invite_token(owner_id: str, customer_id: str) -> str:
    """
    Creates a time-limited invite token that allows a specific
    customer to create their account.

    The token securely stores both owner_id and customer_id,
    eliminating the need for customers to know internal IDs.

    Args:
        owner_id: Owner sending the invitation.
        customer_id: Customer being invited.

    Returns:
        Encoded customer invite JWT.
    """
...
# ───────────────────────────────────────────────────────────────
# Decode Customer Invite Token
# ───────────────────────────────────────────────────────────────
def decode_customer_invite_token(token: str) -> dict:
    """
    Validates and decodes a customer invitation token.

    Args:
        token: Customer invitation JWT.

    Returns:
        Dictionary containing owner_id and customer_id.

    Raises:
        ValueError: If the token is invalid,
        expired, or has the wrong type.
    """
    try:
        # Step 1: Decode JWT
        payload = jwt.decode(
    ...
    # Step 2: Validate token type
    if payload.get("type") != "customer_invite":
        raise ValueError("Wrong token type")

    # Step 3: Return decoded IDs
    return {
...
# ───────────────────────────────────────────────────────────────
def create_customer_access_token(customer_account_id: str) -> str:
    """
    Creates a short-lived JWT access token for a customer account.

    Args:
        customer_account_id: Customer account's unique ID.

    Returns:
        Encoded customer access JWT.
    """
...
# ───────────────────────────────────────────────────────────────
# Create Customer Refresh Token
# ───────────────────────────────────────────────────────────────
def create_customer_refresh_token(customer_account_id: str) -> str:
    """
    Creates a long-lived JWT refresh token for a customer account.

    Args:
        customer_account_id: Customer account's unique ID.

    Returns:
        Encoded customer refresh JWT.
    """
